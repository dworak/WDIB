...
static const LDAtomProperties atomProperties[NUM_ATOMTYPES] = {
...
@interface LDOpenGLRenderer : NSObject 
{
 	GLint backingWidth;
	GLint backingHeight;
...
- (void)renderFrameForMolecule:(LDMolecule *)molecule;
...
- (void)addVertex:(GLfloat *)newVertex forAtomType:(LDAtomType)atomType;
- (void)addIndex:(GLushort *)newIndex forAtomType:(LDAtomType)atomType;
- (void)addIndices:(GLushort *)newIndices size:(unsigned int)numIndices forAtomType:(LDAtomType)atomType;
...
- (void)addAtomToVertexBuffers:(LDAtomType)atomType atPoint:(LD3DPoint)newPoint;
- (void)addBondToVertexBuffersWithStartPoint:(LD3DPoint)startPoint endPoint:(LD3DPoint)endPoint bondColor:(GLubyte *)bondColor bondType:(LDBondType)bondType;

// OpenGL drawing routines
- (void)bindVertexBuffersForMolecule:(LDMolecule *)molecule;
- (void)drawMolecule;
...
// Molecule 3-D geometry generation
- (void)addTextureCoordinate:(GLfloat *)newTextureCoordinate forAtomType:(LDAtomType)atomType;
- (void)addAmbientOcclusionTextureOffset:(GLfloat *)ambientOcclusionOffset forAtomType:(LDAtomType)atomType;
...
- (void)prepareAmbientOcclusionMapForMolecule:(LDMolecule *)molecule;
